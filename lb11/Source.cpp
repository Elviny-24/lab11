...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	w.hbrBackground = (HBRUSH)GetStockObject(WHITE_BRUSH + 3);
	w.lpszClassName = L"My Class";
	RegisterClass(&w);
	hwnd = CreateWindow(L"My Class", L"LB11", WS_OVERLAPPEDWINDOW | WS_BORDER, 10, 0, 1200, 700, NULL, NULL, hInstance, NULL);
...
LONG WINAPI WndProc(HWND hwnd, UINT Message, WPARAM wParam, LPARAM lparam)
{
	...
	case WM_RBUTTONDBLCLK:
	{
		...
		int Width = 300;
		int Height = 300;
		MoveWindow(hwnd, 700, 200, Width, Height, TRUE);
		break;
	}
	break;
